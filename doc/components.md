...
| Class| Description |
| ----- | ----------- |
|\InSquare\OpendxpProcessManagerBundle\Executor\PimcoreCommand |  Executes a pimcore command |
|\InSquare\OpendxpProcessManagerBundle\Executor\ClassMethod  | Initializes a Class and calls a method  |

### Action Classes

| Class| Description |
| ----- | ----------- |
| \InSquare\OpendxpProcessManagerBundle\Executor\Action\Download| Provide a download after a job has finished  |
| \InSquare\OpendxpProcessManagerBundle\Executor\Action\OpenItem| Adds a button to open an item (object/document/asset) after a job has finished  |
| \InSquare\OpendxpProcessManagerBundle\Executor\Action\JsEvent| Executes a custom JavaScript event after a job has finished  |
...
| Class| Description |
| ----- | ----------- |
| \InSquare\OpendxpProcessManagerBundle\Executor\Callback\ExecutionNote | Just an easy callback (provide a text note field) to get started. Callback classes are always job specific ;-)  |

### Logger Classes 

| Class| Description |
| ----- | ----------- |
| \InSquare\OpendxpProcessManagerBundle\Executor\Logger\File | Logs the messages to a file. If no file path is specified, the logs are written to  /website/var/log/process-manager/(MonitoringItem-ID).log |
|\InSquare\OpendxpProcessManagerBundle\Executor\Logger\Console |  The messages are logged to the php stdout (for cli execution) |
|\InSquare\OpendxpProcessManagerBundle\Executor\Logger\Application  | The messages are logged to the Application-Logger. The name of the Configuration is used as component name  |
|\InSquare\OpendxpProcessManagerBundle\Executor\Logger\Email  | The messages are sent per email to the recipient (one email with all logs)|
