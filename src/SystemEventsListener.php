...
class SystemEventsListener implements EventSubscriberInterface
{
    ...
    {
        if (!\OpenDxp::isInstalled() || !$this->installer->isInstalled()) {
            return;
        }

        if (($monitoringItem = InSquareOpendxpProcessManagerBundle::getMonitoringItem()) instanceof \InSquare\OpendxpProcessManagerBundle\Model\MonitoringItem) {
            $error = $e->getError();
    ...
    public function onConsoleTerminate(ConsoleTerminateEvent $e): void
    {
        ...
        if ($e->getExitCode() == 0 && ($monitoringItem = InSquareOpendxpProcessManagerBundle::getMonitoringItem())) {
