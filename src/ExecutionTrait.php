...
trait ExecutionTrait
{
     ...
     * @throws \Exception
     */
    public static function initProcessManager(?int $monitoringId, array $options = []): ?\InSquare\OpendxpProcessManagerBundle\Model\MonitoringItem
    {
        if (!InSquareOpendxpProcessManagerBundle::getMonitoringItem(false) instanceof \InSquare\OpendxpProcessManagerBundle\Model\MonitoringItem) {
            ...
            if ($monitoringId) {
                ...
                InSquareOpendxpProcessManagerBundle::setMonitoringItem($monitoringItem);
            } elseif (getenv(InSquareOpendxpProcessManagerBundle::MONITORING_ITEM_ENV_VAR)) { //check for env passed
                $monitoringId = (int)getenv(InSquareOpendxpProcessManagerBundle::MONITORING_ITEM_ENV_VAR);
                $monitoringItem = MonitoringItem::getById($monitoringId);
                InSquareOpendxpProcessManagerBundle::setMonitoringItem($monitoringItem);
            }
            ...
            $monitoringItem = InSquareOpendxpProcessManagerBundle::getMonitoringItem();
            if ($monitoringId) {
                if ($monitoringItem instanceof \InSquare\OpendxpProcessManagerBundle\Model\MonitoringItem) {
                    ...
                    $values = $config instanceof \InSquare\OpendxpProcessManagerBundle\Model\Configuration ? $config->getExecutorClassObject()->getValues() : $options;
                ...
                if (!defined('OPENDXP_CONSOLE') || !OPENDXP_CONSOLE) {
                    register_shutdown_function(function ($arguments): void {
                        InSquareOpendxpProcessManagerBundle::shutdownHandler($arguments);
                    }, $options);
                }
                InSquareOpendxpProcessManagerBundle::startup($options);
        ...
        self::checkExecutingUser(InSquareOpendxpProcessManagerBundle::getConfiguration()->getAdditionalScriptExecutionUsers());

        return InSquareOpendxpProcessManagerBundle::getMonitoringItem();
    }
    ...
    protected static function doUniqueExecutionCheck(mixed $config, array $options): void
    {
        ...
        if (($count = count($processesRunning)) !== 0) {
            foreach ($processesRunning as $process) {
                ...
                if ($process->getId() != InSquareOpendxpProcessManagerBundle::getMonitoringItem()->getId()) {
                    InSquareOpendxpProcessManagerBundle::getMonitoringItem()->delete();
                }
            }
            InSquareOpendxpProcessManagerBundle::getMonitoringItem()->getLogger()->info('Another process with the PID ' . getmypid() . ' started. Exiting Process:' . getmypid());
            InSquareOpendxpProcessManagerBundle::setMonitoringItem(null);
     ...
     * @return MonitoringItem|null
     */
    public static function getMonitoringItem(): ?\InSquare\OpendxpProcessManagerBundle\Model\MonitoringItem
    {
        return InSquareOpendxpProcessManagerBundle::getMonitoringItem();
    }
    ...
    {
        $configFile = OPENDXP_WEB_ROOT . '/index.php';
