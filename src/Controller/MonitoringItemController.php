...
use InSquare\OpendxpProcessManagerBundle\Executor\Logger\AbstractLogger;
use InSquare\OpendxpProcessManagerBundle\Executor\Logger\Application;
use InSquare\OpendxpProcessManagerBundle\Executor\Logger\File;
...
#[Route(path: '/admin/insquare-opendxp-process-manager/monitoring-item')]
class MonitoringItemController extends UserAwareController
{
    ...
    public function update(Request $request): JsonResponse
    {
        ...
        if ($monitoringItem) {
            if ($monitoringItem->getExecutedByUser() == $this->getCurrentUser()->getId()) {
    ...
    protected function getProcessesForCurrentUser(): MonitoringItem\Listing
    {
        ...
        $list->setCondition('executedByUser = ? and parentId IS NULL AND published = 1 ', [$this->getCurrentUser()->getId()]);
    ...
    protected function getItemData(MonitoringItem $item): array
    {
        ...
        if ($tmp['actions']) {
            ...
            foreach ($actionItems as $i => $v) {
                if ($class = $v['class']) {
                    if (\OpenDxp\Tool::classExists($class)) {
                        $o = new $class();
        ...
        if ($loggers = $item->getLoggers()) {
            foreach ((array)$loggers as $i => $logger) {
                ...
                if (\OpenDxp\Tool::classExists($class::class)) {
                    $logger['index'] = $i;
        ...
        try {
            ...
            if ($loggers = $monitoringItem->getLoggers()) {
                foreach ((array)$loggers as $i => $config) {
                    ...
                    if (\OpenDxp\Tool::classExists($class::class) && $i == $loggerIndex) {
                        /**
    ...
    public function logFileLoggerAction(Request $request, ?Profiler $profiler): Response
    {
        ...
        /**
         * @var \OpenDxp\Security\User\User $tokenUser
         */
    ...
    public function cancelAction(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($pid) {
                ...
                $message = 'Process with PID "' . $pid . '" killed by Backend User: ' . $this->getCurrentUser()->getName();
    ...
    public function restartAction(Request $request, MessageBusInterface $messageBus): JsonResponse
    {
        try {
            ...
            putenv(InSquareOpendxpProcessManagerBundle::MONITORING_ITEM_ENV_VAR . '=' . $monitoringItem->getId());
