...
class File extends AbstractLogger
{
    ...
    public string $extJsClass = 'opendxp.plugin.processmanager.executor.logger.file';
    ...
    public function getGridLoggerHtml(MonitoringItem $monitoringItem, array $actionData): string
    {
        ...
        if (is_readable($logFile)) {
            $icon = ($actionData['icon'] ?? null) ?: '/bundles/opendxpadmin/img/flat-color-icons/file-border.svg';
    ...
    {
        return ($v = $config['filepath'] ?? null) ? OPENDXP_PROJECT_ROOT . $v : $monitoringItem->getLogFile();
    }
}
