opendxp.registerNS("opendxp.plugin.processmanager.executor.action.openItem");
opendxp.plugin.processmanager.executor.action.openItem = new Class.create(opendxp.plugin.processmanager.executor.action.abstractAction,{

    getButton : function(){
        this.button = {
            icon: '/bundles/opendxpadmin/img/flat-color-icons/open_window.svg',
    ...
    executeActionForActiveProcessList : function(actionButtonPanel,actionData,monitoringItem,obj,index){
        if(actionData.executeAtStates.includes(monitoringItem.status)){
            let icons = {
                document : "/bundles/opendxpadmin/img/flat-white-icons/page.svg",
                object : "/bundles/opendxpadmin/img/flat-white-icons/object.svg",
                asset : "/bundles/opendxpadmin/img/flat-white-icons/camera.svg"
            };
            ...
            let button = Ext.create('Ext.Button', {
                ...
                iconCls : ' opendxp_icon_overlay_go ',
                style: (index > 0 ? 'margin-left:5px;' : ''),
                scale: 'small',
                handler: function() {
                    let type = actionData.type;
                    if(type === 'asset'){
                        opendxp.helpers.openAsset(actionData.itemId,actionData.dynamicData.item_type);
                    }
                    if(type === 'object'){
                        opendxp.helpers.openObject(actionData.itemId,actionData.dynamicData.item_type);
                    }
                    if(type === 'document'){
                        opendxp.helpers.openDocument(actionData.itemId,actionData.dynamicData.item_type);
                    }
                }
            });
...
document.addEventListener('processManager.monitoringItemGrid', (e) => {
    ...
    if(e.detail.trigger === 'openItem'){
        ...
        opendxp.helpers["open" + actionType](itemId,itemType);
    }
});