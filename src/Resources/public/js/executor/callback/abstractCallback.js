opendxp.registerNS("opendxp.plugin.processmanager.executor.callback.abstractCallback");
opendxp.plugin.processmanager.executor.callback.abstractCallback = Class.create(opendxp.plugin.processmanager.helper.form, {
    executionUrl: "/admin/insquare-opendxp-process-manager/config/execute",
        ...
        this.buttons = [
            {
                ...
                icon: '/bundles/opendxpadmin/img/flat-color-icons/ok.svg',
                hideLabel: true,
                handler: function () {
                    ...
                    Ext.Ajax.request({
                        url: '/admin/insquare-opendxp-process-manager/callback-settings/save',
                        method: 'post',
                        params: params,
                        ...
                        success: function (response) {
                            ...
                            if (data.success) {
                                opendxp.helpers.showNotification(t("success"), t("plugin_pm_config_callback_save_success"), "success");
                                Ext.getCmp('plugin_pm_callback_settings_panel').store.reload();
                                this.saveSettingsWindow.close();
                            } else {
                                opendxp.helpers.showNotification(t("error"), t("plugin_pm_config_callback_save_error"), "error", data.message);
                            }
                        }.bind(this)
                    });
    ...
    openSaveSettingsTab: function () {
        ...
        if (tab) {
            ...
            this.saveSettingsWindow.on("destroy", function () {
                opendxp.globalmanager.remove(tabId);
            }.bind(this));
        }
        ...
        if (typeof this['getFormItems'] == 'function') {
            ...
            if(this.config){
                ...
                if(preDefinedConfigId){
                    Ext.Ajax.request({
                        url: '/admin/insquare-opendxp-process-manager/callback-settings/list?type=' + this.name + '&id=' + preDefinedConfigId,
            ...
            this.formPanel.getForm().submit({
                ...
                success: function (re, result) {
                    ...
                    if (data.success) {
                        ...
                        opendxp.helpers.showNotification(t("error"), t("plugin_pm_start_error"), "error", data.message);
                    }
                    if (this.grid != null) {
    ...
    getConfigSelection : function () {
        var configStore = new Ext.data.Store({
            autoLoad : true,
            proxy: {
                url: '/admin/insquare-opendxp-process-manager/callback-settings/list?type=' + this.name,
                type: 'ajax',
                reader: {
                    type: 'json',
                    rootProperty: "data"
