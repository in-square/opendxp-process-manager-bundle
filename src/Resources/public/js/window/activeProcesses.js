opendxp.registerNS("opendxp.plugin.processmanager.window.activeProcesses");

opendxp.plugin.processmanager.window.activeProcesses = Class.create({
    displayList : [],
    toast : null,
    ...
    _updatePrimaryBtn: function(item) {
        ...
        if(item.isAlive){
            button.setIcon('/bundles/opendxpadmin/img/flat-color-icons/cancel.svg')
            button.setTooltip(t('plugin_pm_stop'));
        }else{
            button.setIcon('/bundles/opendxpadmin/img/flat-color-icons/approve.svg');
        ...
        button.handler = function() {
            if(item.isAlive){
                Ext.Ajax.request({
                    url: '/admin/insquare-opendxp-process-manager/monitoring-item/cancel',
                    method : 'get',
                    params : {
                        id : item.id
                    },
                ...
                Ext.Ajax.request({
                    url: '/admin/insquare-opendxp-process-manager/monitoring-item/update',
                    method : 'post',
                    params : {
                        id : item.id,
                        published : false
                ...
                for(let i = 0; i < item.actionItems.length; i++){
                    ...
                    if(actionData.executeAtStates.includes(item.status)){
                        if(actionData.dynamicData && actionData.dynamicData.extJsClass){
                            if(  (actionData.dynamicData.extJsClass == 'opendxp.plugin.processmanager.executor.action.download')
                               ||(actionData.dynamicData.extJsClass == 'opendxp.plugin.processmanager.executor.action.openItem')){
        ...
        if(this.toast){
            ...
            if(el){
                ...
                if(data.active){
                    ...
                    s = '(<img src="/bundles/opendxpadmin/img/flat-color-icons/approve.svg" alt="" class="active_processes_finished_icon">' + t('plugin_pm_all_finished') +')'
                }
                el.innerHTML = s;
            }
        }
    ...
    requestServerData: function() {
        ...
        Ext.Ajax.request({
            url: '/admin/insquare-opendxp-process-manager/monitoring-item/list-processes-for-user',
            success: function (content) {
                        ...
                        this.toast = Ext.create('Ext.window.Toast', {
                            ...
                            tools:[{
                                ...
                                handler: function(event, toolEl, panelHeader) {
                                    Ext.Ajax.request({
                                        url: '/admin/insquare-opendxp-process-manager/monitoring-item/update-all-user-monitoring-items',
                                        method : 'post',
                                        params : {
                                            published : false
                                        },
