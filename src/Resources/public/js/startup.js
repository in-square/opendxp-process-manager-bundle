...
opendxp.registerNS('opendxp.plugin.processmanager');

opendxp.plugin.processmanager = Class.create({
    config : {},

    getClassName: function() {
        return "opendxp.plugin.processmanager";
    },
    ...
    createNavigationEntry: function (e) {
        ...
        if (menu.extras &&
            ...
            Ext.Ajax.on({requestexception: function (conn, response, options) {
                if(response.request.url.startsWith('/admin/insquare-opendxp-process-manager/monitoring-item/list') && options.action === "read") {
    ...
    showProcessManager : function (config){
        ...
        if (opendxp.globalmanager.get("plugin_pm_cnf")) {
            Ext.getCmp("opendxp_panel_tabs").setActiveItem("opendxp_plugin_pm_panel");
            if(config.activeTab){
                Ext.getCmp("opendxp_plugin_pm_panel").setActiveTab(config.activeTab);
            }
        } else {
            opendxp.globalmanager.add("plugin_pm_cnf", new opendxp.plugin.processmanager.panel.general(config));
        }
    },

    getConfig : function(){
        Ext.Ajax.request({
            url: '/admin/insquare-opendxp-process-manager/index/get-plugin-config',
    ...
    initActiveProcesses : function(){
        if(opendxp.globalmanager.get("user").isAllowed("plugin_pm_permission_execute")){
            this.activeProcesses = new opendxp.plugin.processmanager.window.activeProcesses();
    ...
    addShortcutMenu : function () {
        if(opendxp.globalmanager.get("user").isAllowed("plugin_pm_permission_execute")){
            ...
            if(this.config.shortCutMenu){
                ...
                var insertPoint = Ext.get("opendxp_menu_settings");
                if(!insertPoint) {
                    var dom = Ext.dom.Query.select('#opendxp_navigation ul li:last');
                ...
                this.navEl = Ext.get(
                    insertPoint.insertHtml(
                        "afterEnd",
                        '<li id="plugin_pm_shortcut_menu" class="opendxp_menu_item" data-menu-tooltip="' + t('plugin_pm')+'">'  +
    ...
    monitoringItemRestart : function(id){
        Ext.Ajax.request({
            url: '/admin/insquare-opendxp-process-manager/monitoring-item/restart?id=' + id,
            success: function(response, opts) {
                var data = Ext.decode(response.responseText);
                if(data.success){
                    opendxp.helpers.showNotification(t("success"), t("plugin_pm_restart_success"), "success");
                    this.activeProcesses.refreshTask.start();
                    Ext.getCmp("plugin_pmmonitoring_item_list_panel").store.reload();
                }else{
                    opendxp.helpers.showNotification(t("error"), t("plugin_pm_restart_error"), "error",data.message);
                }
            }.bind(this)
        });
    },
    ...
    monitoringReopenCallback : function(configurationId,monitoringItemId){
        Ext.Ajax.request({
            url: '/admin/insquare-opendxp-process-manager/monitoring-item/get-by-id?id=' + monitoringItemId,
            success: function(response, opts) {
    ...
    monitoringItemCancel : function (id) {
        Ext.Ajax.request({
            url: '/admin/insquare-opendxp-process-manager/monitoring-item/cancel?id=' + id,
            success: function(response, opts) {
                var data = Ext.decode(response.responseText);
                if(data.success){
                    opendxp.helpers.showNotification(t("success"), t("plugin_pm_cancel_success"), "success");
                    Ext.getCmp("plugin_pmmonitoring_item_list_panel").store.reload();
                }else{
                    opendxp.helpers.showNotification(t("error"), t("plugin_pm_cancel_error"), "error",data.message);
                }
            }.bind(this)
        });
    },
    ...
    openLogId : function (id){
        ...
        let pm = opendxp.globalmanager.get("plugin_pm_cnf");
    ...
    download : function(id,accessKey){
        var url = '/admin/insquare-opendxp-process-manager/index/download?id='+ id +'&accessKey=' + accessKey;
        opendxp.helpers.download(url);
    },

    executeJob: function (id,monitoringItemData) {
        Ext.Ajax.request({
            url: '/admin/insquare-opendxp-process-manager/config/get-by-id?id=' + id,
            success: function (response) {
                ...
                if (data.success) {
                    ...
                    if (callbackClass) {
                        ...
                        callbackClass = new opendxp.plugin.processmanager.executor.callback.default();
                    }
                    ...
                    opendxp.helpers.showNotification(t("error"), t("plugin_pm_error_process_manager"), "error", data.message);
                }
            }.bind(this)
        });
    }
...
const processmanagerPlugin = new opendxp.plugin.processmanager();
...
document.addEventListener('processManager.monitoringItemGrid', (e) => {
    ...
    if(e.detail.trigger === 'monitoringReopenCallback'){
        ...
        let tmpObject = new opendxp.plugin.processmanager.executor.logger.file();
